...
**증상:**
```
1. 이번 주 단어: [sunny, cloudy]
2. "복습으로 이동" 클릭
3. 복습 단어: [sunny, cloudy]
4. 🔄 새로고침 (GitHub CSV 불러오기)
5. 결과: 이번 주와 복습에 같은 단어 중복 발생
```

**원인:**
- GitHub 불러오기 시 중복 체크가 각 타입(current/review)별로만 수행됨
- 복습 단어에 있는 단어를 GitHub에서 current로 불러오면 중복 추가됨

**해결:**
- 중복 체크 로직을 **이번 주 + 복습 전체**에서 수행하도록 수정
- 어느 목록에든 이미 있는 단어는 추가하지 않음
...
if (isDuplicate && mode === 'add') continue; // 스킵
```
...
2. "복습으로 이동" 클릭
3. 복습 단어: [sunny, cloudy]
4. 🔄 새로고침 (GitHub: sunny, cloudy with type=current)
5. 중복 감지 → 추가 안 됨
6. 결과: 복습에만 유지, 중복 없음 ✅
```
...
**2025-01-25 (ver 0.2.1 - 버그 수정)**
- 중복 단어 생성 버그 수정
- 새로고침 시 이번 주 + 복습 전체에서 중복 체크
- 이슈: "이번 주 → 복습 이동 후 새로고침하면 중복 발생" 해결
