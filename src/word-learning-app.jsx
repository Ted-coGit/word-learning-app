...
const WordLearningApp = () => {
  ...
  const [cardWordList, setCardWordList] = useState([]); // 카드에서 탐색할 단어 리스트
  ...
  const deleteWord = (id) => {
    ...
    const confirmDelete = window.confirm(
      `"${wordToDelete.english}"을(를) 삭제하시겠습니까?`
    );
  ...
  const deleteReviewWord = (id) => {
    const wordToDelete = reviewWords.find(w => w.id === id);
    if (!wordToDelete) return;
  ...
  // 개별 단어를 복습으로 이동
  const moveWordToReview = (id) => {
    const wordToMove = currentWords.find(w => w.id === id);
    if (!wordToMove) return;
    
    // 복습 단어에 추가
    const updatedReview = [...reviewWords, wordToMove];
    saveReviewWords(updatedReview);
    
    // 이번 주 단어에서 제거
    const updatedCurrent = currentWords.filter(w => w.id !== id);
    saveCurrentWords(updatedCurrent);
  };

  // 개별 단어를 이번 주로 이동
  const moveWordToCurrent = (id) => {
    const wordToMove = reviewWords.find(w => w.id === id);
    if (!wordToMove) return;
    
    // 이번 주 단어에 추가
    const updatedCurrent = [...currentWords, wordToMove];
    saveCurrentWords(updatedCurrent);
    
    // 복습 단어에서 제거
    const updatedReview = reviewWords.filter(w => w.id !== id);
    saveReviewWords(updatedReview);
  };
...
초등학생이 이해하기 쉽게 설명해주세요.`
  ...
  const goToNextCard = async () => {
    if (currentCardIndex < cardWordList.length - 1) {
      const nextIndex = currentCardIndex + 1;
      const nextWord = cardWordList[nextIndex];
      setCurrentCardIndex(nextIndex);
      setSelectedWord(nextWord);
      await fetchWordDetails(nextWord);
    }
  };
  ...
  const goToPrevCard = async () => {
    if (currentCardIndex > 0) {
      ...
      const prevWord = cardWordList[prevIndex];
      setCurrentCardIndex(prevIndex);
      setSelectedWord(prevWord);
      await fetchWordDetails(prevWord);
    }
  };
                    ...
                    {currentWords.map((word) => (
                      ...
                      >
                        <div 
                          ...
                          onClick={() => openWordCard(word, currentWords)}
                        >
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => moveWordToReview(word.id)}
                            className="text-orange-500 hover:text-orange-600 flex-shrink-0 text-sm px-2 py-1 rounded hover:bg-orange-50"
                            title="복습으로 이동"
                          >
                            📚
                          </button>
                    ...
                    {reviewWords.length === 0 ? (
                      ...
                      reviewWords.map((word) => (
                        ...
                        >
                          <div 
                            className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer"
                            onClick={() => openWordCard(word, reviewWords)}
                          >
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => moveWordToCurrent(word.id)}
                              className="text-green-500 hover:text-green-600 flex-shrink-0 text-sm px-2 py-1 rounded hover:bg-green-50"
                              title="이번 주로 이동"
                            >
                              📗
                            </button>
